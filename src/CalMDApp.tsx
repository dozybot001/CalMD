...
export default function App() {
  ...
  const documentCardRef = useRef<HTMLDivElement | null>(null);
  ...
  async function loadFile(file: File | null) {
    ...
    if (!isAcceptedFile(file)) {
      setErrorMessage("CalMD 目前支持 .md、.markdown 和 .txt 文件。");
      return;
    }
  ...
  async function handleExportLongImage() {
    ...
    setErrorMessage("");
    setIsLongImageExporting(true);

    try {
      await exportLongImage({
        element: documentCardRef.current,
    ...
    >
      ...
      <main className="layout">
        <aside className="side-rail">
          <div className="control-slot">
            <ReaderControls
              ...
              isLongImageExporting={isLongImageExporting}
              fileName={documentState.fileName}
        ...
        <section className="reader-panel">
          <div ref={documentCardRef} className="document-card">
            {isSearchOpen ? (
